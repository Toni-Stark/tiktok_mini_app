...
export default function Hobby() {
  ...
  const chooseItem = (id) => {
    let index = current.findIndex((item)=>{return item === id});
    let list = [...current];
    if(index>=0){
       list.splice(index, 1);
    } else {
    ...
    <View className="index">
      ...
      <View className="index_content">
                        ...
                        item.list.map((it, ind)=>{
                          ...
                          return (
                            <View
                              ...
                              style={{borderColor: bool>=0?"#fcc547":"#3c4252"}}
                              onClick={()=>{chooseItem(it.id)}}
                            >{it.title}</View>
                          )
                        })
                      }
          ...
          <View className="index_content_btn">
            <View className="index_content_btn_text" style={{background:current.length>0?"linear-gradient(to right, #a829e8, #3c6fef)":"#3c4252"}}>
              已选{option.count}个
            </View>
          </View>
      </View>
